...
void DataTxt::init()
{
	...
	for (int i = 1; i < 7; ++i)
	{
		...
		QString path = PATH + QString::number(i, 10) + "/" + "day" + QString::number(m_day, 10)+".txt";
		QFile file(path);
...
QString DataTxt::getOneData()
{
	...
	for (int i = 1; i < 7; ++i)
	{
		QString record = m_pounds[i-1][m_number];
		...
		int truewatertmp = m_water_temp + diff;
		truewatertmp = (truewatertmp <= 0? 1 : truewatertmp);
		data.append(QString::number(truewatertmp, 10));
		data.append("', 'id':'");
		data.append(QString::number(i, 10));
		data.append("', 'companyid':'1601'},");
		//光照 没有光照
